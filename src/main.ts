...
function handlePaletteChange(e: Event) {
  ...
  switch (target.value) {
    ...
    case PALETTES.OPEN_PROPS:
      ...
      break;
    // Additional palettes can be added here in the future
    default:
      console.warn(`Unhandled palette: ${target.value}`);
  }
...
function setColor(pickedColor: string) {
  if (isHDRMode) {
...
function createQuickPickButton() {
  const quickPickBtn = document.createElement('button');
  quickPickBtn.id = 'quick-pick';
  quickPickBtn.className = 'custom-btn';
...
if ('EyeDropper' in window) {
  const quickPickBtn = createQuickPickButton();

  const parentElement = document.querySelector('.button-container');
  parentElement?.appendChild(quickPickBtn);

  quickPickBtn.addEventListener('click', async () => {
    if (colorPicker) {
      colorPicker.value = await startEyeDropper();
